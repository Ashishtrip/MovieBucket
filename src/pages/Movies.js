...
/**
 * Movies page – displays paginated popular movies fetched from TMDB.
 * Scrolls to the top of the page on every page change.
 */
const Movies = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const { data, loading, error } = useApiFetch(getPopular, 'movie', currentPage);
  ...
  return (
    <div className="container mx-auto px-4 lg:px-8 py-8">
      ...
      {loading ? (
        ...
        <>
          <MediaGrid
            items={results.map((m) => ({ ...m, media_type: 'movie' }))}
